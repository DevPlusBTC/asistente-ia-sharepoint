...
GRAPH="https://graph.microsoft.com/v1.0"; ROOT="BI/Data BI BDD"
...
def main():
    t=token()
    root=g(f"{GRAPH}/me/drive/root:/{ROOT}",t)
    items=g(f"{GRAPH}/me/drive/items/{root['id']}/search(q='.xlsx')",t).get("value",[])
    print(f"archivos: {len(items)}")
    # TODO fase 2: leer worksheets chunked (1000 filas) y tokenizar SENSITIVE
